...
class CreateConsultationNodeCmd {
  ...
  /// Returns a new [CreateConsultationNodeCmd] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static CreateConsultationNodeCmd? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "CreateConsultationNodeCmd[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CreateConsultationNodeCmd[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<CreateConsultationNodeCmd>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <CreateConsultationNodeCmd>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = CreateConsultationNodeCmd.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, CreateConsultationNodeCmd> mapFromJson(dynamic json) {
    final map = <String, CreateConsultationNodeCmd>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = CreateConsultationNodeCmd.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of CreateConsultationNodeCmd-objects as value to a dart map
  static Map<String, List<CreateConsultationNodeCmd>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<CreateConsultationNodeCmd>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = CreateConsultationNodeCmd.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
