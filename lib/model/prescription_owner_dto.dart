...
class PrescriptionOwnerDto {
  ...
  @override
  String toString() => 'PrescriptionOwnerDto[id=$id, created=$created, modified=$modified, reminderCount=$reminderCount, item=$item]';
  ...
  static Map<String, List<PrescriptionOwnerDto>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PrescriptionOwnerDto.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
