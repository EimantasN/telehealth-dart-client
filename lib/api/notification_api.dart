...
class NotificationApi {
  ...
  /// Performs an HTTP 'GET /api/Notification/NotificatioModels' operation and returns the [Response].
  Future<Response> notificationModelsWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/api/Notification/NotificatioModels';
  ...
  Future<NotificationModels?> notificationModels() async {
    final response = await notificationModelsWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'NotificationModels',) as NotificationModels;
