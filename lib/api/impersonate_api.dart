...
class ImpersonateApi {
  ...
  /// Performs an HTTP 'GET /api/Impersonate/Status' operation and returns the [Response].
  Future<Response> impersonateStatusWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/api/Impersonate/Status';
  ...
  Future<ImpersonateStatusDto?> impersonateStatus() async {
    final response = await impersonateStatusWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'ImpersonateStatusDto',) as ImpersonateStatusDto;
