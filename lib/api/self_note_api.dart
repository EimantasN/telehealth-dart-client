...
class SelfNoteApi {
  ...
  /// Performs an HTTP 'DELETE /api/SelfNote/Delete' operation and returns the [Response].
  /// Parameters:
  ///
  /// * [bool] isAll:
  ///
  ...
  ///
  /// * [List<int>] excluded:
  Future<Response> selfNoteDeleteWithHttpInfo({ bool? isAll, List<int>? included, List<int>? excluded, }) async {
    // ignore: prefer_const_declarations
    final path = r'/api/SelfNote/Delete';
  ...
  ///
  /// * [List<int>] excluded:
  Future<bool?> selfNoteDelete({ bool? isAll, List<int>? included, List<int>? excluded, }) async {
    final response = await selfNoteDeleteWithHttpInfo( isAll: isAll, included: included, excluded: excluded, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
